...
-- Funcion para id del cargue
CREATE SEQUENCE load_id_seq START WITH 1 INCREMENT BY 1;
CREATE OR REPLACE FUNCTION generate_load_id RETURN VARCHAR2 AS
    l_counter NUMBER;
    l_load_id VARCHAR2(10);
BEGIN
    -- Obtener el próximo número de secuencia o lógica para generar el ID
    SELECT load_id_seq.NEXTVAL INTO l_counter FROM DUAL;

    -- Formato del ID (por ejemplo, agregar ceros a la izquierda)
    l_load_id := LPAD(TO_CHAR(l_counter), 4, '0');

    RETURN l_load_id;
END generate_load_id;